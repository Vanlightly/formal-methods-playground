    -------------------------------- MODULE swim_stats_single_config --------------------------------
...
CONSTANTS Nil,                      \* Empty numeric value
          ...
          NumMembers,              \* The set of possible members
...
ASSUME DeadMemberCount < NumMembers
ASSUME NewMemberCount < NumMembers
ASSUME UnjoinedMemberCount < NumMembers
    ...
    \E exec_id \in { JavaTime } :
        LET max_stats_round == MaxRound
           ...
           IN
            ...
            /\ \A r \in 1..max_stats_round :
                IF r = max_stats_round 
                THEN PrintT("infective_states_count" \o cfg_str \o ToString(r) \o "," \o ToString(TotalInfectiveStates(DisseminationLimit, peer_states, Member)))
                ELSE PrintT("infective_states_count" \o cfg_str \o ToString(r) \o "," \o ToString(TLCGet(infective_states_ctr(r))))
            /\ \A r \in 1..max_stats_round :
                IF r = max_stats_round 
                THEN PrintT("infectivity" \o cfg_str \o ToString(r) \o "," \o ToString(TotalInfectivity(DisseminationLimit, peer_states, Member)))
    ...
    IN
        /\ \A member \in Member : 
            CSVWrite("%1$s,%2$s,%3$s,%4$s,%5$s,%6$s,%7$s,%8$s,%9$s,%10$s,%11$s,%12$s,%13$s,%14$s,%15$s,%16$s,%17$s",
                <<TLCGet("stats").traces, TLCGet(run_rand), TLCGet(run_ctr), 
                    member, ReceivedMessageLoad(member), ReceivedProbeMessageLoad(member),
                    ReceivedProbeRequestMessageLoad(member),
                    NumMembers, DeadMemberCount, NewMemberCount, SuspectTimeout, DisseminationLimit,
                    MaxUpdatesPerPiggyBack, LoseEveryNth, PeerGroupSize, InitialContacts, 
                    MaxRound>>,
                MemberStatsCSV)
        /\ \A r \in 1..max_stats_round : 
            ...
            IN
                CSVWrite("%1$s,%2$s,%3$s,%4$s,%5$s,%6$s,%7$s,%8$s,%9$s,%10$s,%11$s,%12$s,%13$s,%14$s,%15$s,%16$s,%17$s,%18$s,%19$s,%20$s,%21$s,%22$s,%23$s,%24$s,%25$s,%26$s,%27$s",
                    <<TLCGet("stats").traces, TLCGet(run_rand), TLCGet(run_ctr), 
                        ...
                        NumMembers, DeadMemberCount, NewMemberCount, SuspectTimeout, DisseminationLimit,
                        MaxUpdatesPerPiggyBack, LoseEveryNth, PeerGroupSize, InitialContacts, MaxRound>>,
                    RoundStatsCSV)
...
StartSim ==
    /\ ~SimStarted
    /\ TLCSet(run_rand, RandomElement(1..100000))
    /\ UNCHANGED vars
...
EndSim ==
    ...
    /\ IF PrintStatsOnDeadlock = TRUE 
       THEN IF IsConverged(incarnation, peer_states, Member, Nil, DeadState, AliveState) 
            ...
            ELSE IF CannotConvergeOnJoined THEN PrintNoConvergence ELSE PrintT("bad-result") 
       ELSE TRUE
    /\ sim_status' = 1
    /\ TLCSet(run_ctr, TLCGet(run_ctr) + 1)
    /\ UNCHANGED <<incarnation, peer_states, messages, pending_direct_ack, pending_indirect_ack, probe_ctr, round, initial_state_vars>>
...
SendDirectProbe(member, peer) ==
    ...
    /\ LET gossip_to_send == SelectOutgoingGossip(member, peer, peer_states[member])
       IN
        ...
        /\ RegisterPendingDirectAck(member, peer)
        /\ UNCHANGED <<incarnation, pending_indirect_ack, round, sim_status, initial_state_vars >>
...
ReceiveProbe ==
    ...
    /\ UNCHANGED <<round, pending_direct_ack, pending_indirect_ack, probe_ctr, sim_status, initial_state_vars >>
    ...
    /\ \E msg \in DOMAIN messages :
        ...
        /\ LET merged_peer_state == MergeGossipWithCurrentState(msg.dest, 
           ...
           IN
            ...
            /\ UNCHANGED << pending_indirect_ack, probe_ctr, sim_status, initial_state_vars>>
...
SendProbeRequest ==
    ...
    /\ \E msg \in DOMAIN messages :
        ...
        /\ UNCHANGED <<incarnation, probe_ctr, round, sim_status, initial_state_vars>>
...
NoPeersForProbeRequest ==
    ...
    /\ \E msg \in DOMAIN messages :
        ...
        /\ UNCHANGED <<incarnation, messages, probe_ctr, pending_indirect_ack, sim_status, initial_state_vars>>    
        ...
        /\ LET merged_peer_state == MergeGossipWithCurrentState(msg.dest, 
           ...
           IN LET gossip_to_send == SelectOutgoingGossip(msg.dest, msg.target, merged_peer_state)
              IN
                ...
                /\ RecordGossipStats(msg.dest, msg.source, msg.gossip, FALSE)
    /\ UNCHANGED << pending_direct_ack, pending_indirect_ack, probe_ctr, round, sim_status, initial_state_vars >>
           ...
           IN
            ...
            /\ IF round[msg.dest] # msg.round \/ pending_indirect_ack[msg.dest] = Nil 
               ...
               ELSE /\ IncrementRound(msg.dest)
                    ...
                    /\ RecordGossipStats(msg.dest, msg.source, msg.gossip, TRUE) 
            /\ MessageProcessed(msg)
            /\ UNCHANGED << pending_direct_ack, probe_ctr, sim_status, initial_state_vars>>
    ...
    /\ \E member \in Member :
        ...
        /\ LET pr == CHOOSE msg \in DOMAIN messages :
           ...
           IN
                ...
                /\ RecordStateStats
                /\ UNCHANGED <<incarnation, messages, pending_direct_ack, probe_ctr, sim_status, initial_state_vars>>
...
MemberJoins ==
    \E member \in Member :
        ...
        /\ \E contact_peers \in SUBSET Member : 
            ...
            /\ Cardinality(contact_peers) = InitialContacts
            /\ peer_states' = [peer_states EXCEPT ![member] =
