...
public class swim_stats {
	...
	public static Value IsConverged(final FcnRcdValue inc,
		...
		final int memSize = mem.size();
		for (int member = 0; member < memSize; member++) {
			final int realState = inc.values[member].equals(nil) ? deadState.val : aliveState.val;
			int real = 0;
			int dead = 0;
			int livePeers = 0;

			for (int peer = 0; peer < memSize; peer++) {
				if(member == peer)
					continue;
...
StateCount(state, target_peer_states) ==
	 ...
	 */
	public static Value StateCount(final IntValue state, final FcnRcdValue tps, final IntervalValue mem) {
		assert tps.isNormalized();
		...
		int memCount = mem.size();
		for (int i = 0; i < memCount; i++) {
			final IntValue di = (IntValue) mem.elementAt(i);
			for (int j = 0; j < memCount; j++) {
				final IntValue dj = (IntValue) mem.elementAt(j);
...
MemberCount(state, target_peer_states) ==
	 ...
	 */
	public static Value MemberCount(final IntValue state, final FcnRcdValue tps, final IntervalValue mem) {
		assert tps.isNormalized();
		assert mem.isNormalized();
		...
		int memCount = mem.size();
		for (int member = 0; member < memCount; member++) {
			for (int peer = 0; peer < memCount; peer++) {
				if(member == peer)
					continue;
				final FcnRcdValue frv = (FcnRcdValue) tps.values[peer];
				final RecordValue rv = (RecordValue) frv.values[member];
...
IsNewRoundTransitionStep(inc, r1, r2, nil, mem, pstates, dead_state) ==
	...
	public static Value TotalInfectivity(final IntValue disseminationsLimit,
										 final FcnRcdValue tps,
										 final IntervalValue mem) {
		assert tps.isNormalized();
	...
	public static Value TotalInfectiveStates(final IntValue disseminationsLimit,
											 final FcnRcdValue tps,
											 final IntervalValue mem) {
		assert tps.isNormalized();
