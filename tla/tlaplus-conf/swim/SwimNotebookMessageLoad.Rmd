...
msg_load_all_df <- select(raw_member_msg_load_df, NumMembers, RunRand, Run, Round, Member, 
...
melted <- melt(msg_load_all_df, id = c("NumMembers", "RunRand", "Run", "Round", "Member"),
...
first_detect_df <- select(filter(raw_round_stats_df, DeadMembersCount > 0),
                                 NumMembers, RunRand, Run, Round)
first_detect_grouped <- group_by(first_detect_df, NumMembers, RunRand, Run)
...
first_detect_mem_df <- select(filter(raw_msgload_stats_df2, DeadStates > 0),
                                 NumMembers, RunRand, Run, Round, Member)
first_detect_mem_grouped <- group_by(first_detect_mem_df, NumMembers, RunRand, Run, Member)
