...
EXTENDS Sequences, Integers, FiniteSets, FiniteSetsExt, Naturals, TLC, TLCExt, CSV, IOUtils
...
ExistsOtherUnderActiveApp(a) ==
    \A a1 \in app :
        \/ a1 = a
        \/ /\ a1 # a
           /\ IdealNumber(a) <= AppActiveCount(a)
...
\* it is subscribed to all queues
CanPerformCheck(a) ==
    /\ ~IsBalanced
    /\ \A a1 \in app : 
...
PerformNonActiveReleaseCheck(a) ==
    ...
    /\ LET release_count == AppActiveCount(a) - IdealNumber(a) 
       IN
            ...
            ELSE IF release_count = 0 /\ ExistsOtherUnderActiveApp(a) THEN
                /\ ReleaseNonActiveQueues(a)
                /\ StdOut(<<"PerformNonActiveReleaseCheck", a, release_count, "release non-active">>)
            ELSE
                /\ IncrementMetrics(a, {}, 0)
                /\ StdOut(<<"PerformNonActiveReleaseCheck", a, release_count>>)
                /\ UNCHANGED << app_queues, subscriber_queue, active >>
...
TestInv ==
    ...
    \* IF TLCGet("level") > 1
    \* THEN
    \*     /\ PrintT(<<"QA", Cardinality(app), Cardinality(queue)>>)
    \*     /\ \A a \in app : 
    \*             PrintT(<<"Ideal", a, IdealNumber(a),
    \*                      IdealNumberVals(a),
    \*                      AppActiveCount(a)>>)
    \*     /\ \A a \in app : PrintT(<<"Current", AppActiveCount(a), {q \in queue : active[q] = a}>>)
    \* ELSE TRUE
